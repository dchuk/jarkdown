...
from jira_download_pkg.jira_api_client import JiraApiClient
from jira_download_pkg.attachment_handler import AttachmentHandler
from jira_download_pkg.markdown_converter import MarkdownConverter
from jira_download_pkg.exceptions import (
    JiraApiError,
    AuthenticationError,
    IssueNotFoundError,
...
class TestJiraApiClient:
    ...
    def test_init_with_valid_credentials(self):
        ...
        client = JiraApiClient('example.atlassian.net', 'test@example.com', 'test-token-123')
    ...
    def test_network_error(self, api_client, mocker):
        ...
        mocker.patch.object(api_client.session, 'get', side_effect=requests.exceptions.ConnectionError('Network error'))
...
class TestAttachmentHandler:
    ...
    def test_single_attachment_download(self, attachment_handler, mocker, tmp_path):
        ...
        mocker.patch.object(attachment_handler.api_client, 'download_attachment_stream', return_value=mock_response)
    ...
    def test_download_error_raises_exception(self, attachment_handler, mocker, tmp_path):
        ...
        mocker.patch.object(attachment_handler.api_client, 'download_attachment_stream', 
    ...
    def test_multiple_attachments_download(self, attachment_handler, mocker, tmp_path):
        ...
        attachments = [
            {'filename': 'file1.pdf', 'content': 'url1', 'mimeType': 'application/pdf', 'size': 1024},
            {'filename': 'file2.jpg', 'content': 'url2', 'mimeType': 'image/jpeg', 'size': 2048},
        ]
...
class TestMarkdownConverter:
    ...
    def test_image_url_replacement(self, markdown_converter):
        ...
        attachments = [{'filename': 'screenshot.png', 'original_filename': 'screenshot.png'}]
    ...
    def test_link_url_replacement(self, markdown_converter):
        ...
        attachments = [{'filename': 'document.pdf', 'original_filename': 'document.pdf'}]
    ...
    def test_complete_markdown_structure(self, markdown_converter, issue_with_attachments):
        """Test complete markdown generation"""
        attachments = [
            {'filename': 'screenshot.png', 'mime_type': 'image/png', 'original_filename': 'screenshot.png'},
            {'filename': 'document.pdf', 'mime_type': 'application/pdf', 'original_filename': 'document.pdf'}
        ]
        
        result = markdown_converter.compose_markdown(issue_with_attachments, attachments)
    ...
    def test_attachments_section_formatting(self, markdown_converter):
        ...
        attachments = [
            {'filename': 'image.png', 'mime_type': 'image/png', 'original_filename': 'image.png'},
            {'filename': 'image.jpg', 'mime_type': 'image/jpeg', 'original_filename': 'image.jpg'},
            {'filename': 'document.pdf', 'mime_type': 'application/pdf', 'original_filename': 'document.pdf'},
            {'filename': 'data.csv', 'mime_type': 'text/csv', 'original_filename': 'data.csv'},
        ]
